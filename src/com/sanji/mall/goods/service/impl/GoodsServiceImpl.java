...
public class GoodsServiceImpl implements GoodsService {
	...
	public List<Goods> getHotSale(int page, int size, String brandName) {
		...
		int end = page * size;
		List<Goods> goodss = goodsMapper.gainHotByBrandName(start, end,
				brandName);
		return goodss;
	}

	@SuppressWarnings("rawtypes")
	public List<Map<String, Object>> getBjd(String brandName, String userId,
			String machineType) {
		...
		List<Map<String, Object>> selectBjd = goodsMapper.selectBjd(brandName,
				userId, regionId, machineType, user.getUsername());

		for (int i = 0; i < selectBjd.size(); i++) {
			if (selectBjd.get(i).get("GOODS_NAME").equals(
					"酷派 酷派cool 1合约机B2B")) {
				if (City != 2311 && City != 2293 && City != 2331
						&& City != 2214) {
					selectBjd.remove(i);
				}
			}
			if (selectBjd.get(i).get("GOODS_NAME").equals("奇酷  奇酷360 F4合约机")
					&& selectBjd.get(i).get("GOODS_NAME")
							.equals("奇酷  奇酷360 F4合约机（可调拨串号）")) {
			...
			if (selectBjd.get(i).get("GOODS_NAME").equals("华为 华为畅享6（合约机）B2B")
					&& selectBjd != null) {
			...
			if (selectBjd.get(i).get("GOODS_NAME").equals("魅族 魅蓝E 移动标准版（济南可调拨串号）")
					|| selectBjd.get(i).get("GOODS_NAME").equals("荣耀 荣耀5C 标配（济南可调拨串号）")) {
				if (City != 2183 && City != 2324 && City != 2243 && City != 2230 && City != 2298) {
	...
	public List<Goods> gainAllGoodsInfo2(String memberId, String goodsNum,
			String area, String userName) {
		return goodsMapper
				.gainAllGoodsInfo2(memberId, goodsNum, area, userName);
	}

	@Override
	public List<Goods> gainAllShoppingRush(String memberId, String goodsNum,
			String area, String userName) {
