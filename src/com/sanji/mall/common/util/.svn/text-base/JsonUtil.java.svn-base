...
public class JsonUtil {
	...
	public static Object getObject4JsonString(String jsonString, Class pojoCalss) {
		if (ToolsUtil.isNullOrEmpty(jsonString))
			return null;
		Object pojo;
					...
					if (fields != null && fields.length > 0) {
						...
						for (int j = 0; j < fields.length; j++) {
							...
							for (int t = 0; t < methods.length; t++) {
								String temp = "get" + fields[j].getName().toUpperCase().substring(0, 1) + fields[j].getName().substring(1);
								// System.out.println(" ####################"+
								// temp);
	 ...
	 * @return
	 */
	public static String getList2JsonStr(List data, Class pojoCalss, String idField, String nameField) {
		StringBuffer result = new StringBuffer();
		result.append("[");
		if (data != null && data.size() > 0) {
			for (int i = 0; i < data.size(); i++) {
				...
				try {
					...
					if (obj[i] instanceof String) {
						...
						if (obj[i].toString() != null) {
							s = obj[i].toString().replace("\"", "\\\"").replace("'", "\\'").replace("\n", "<br/>").replace("\b", "\\b").replace("\f", "\\f").replace("\r", "")
									.replace("\t", "");
						}
						result.append(s);
