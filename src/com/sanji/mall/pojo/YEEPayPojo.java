/**  
* @Title: YEEPayPojo.java
* @Package com.sanji.mall.pojo
* @Description: TODO(用一句话描述该文件做什么)
* @author ZhouZhangbao  
* @date 2014-11-24 下午7:03:33
* @version V1.0  
*/
package com.sanji.mall.pojo;

/**
 * @ClassName: YEEPayPojo
 * @Description: TODO(这里用一句话描述这个类的作用)
 * @author ZhouZhangbao
 * @date 2014-11-24 下午7:03:33
 */
public class YEEPayPojo {
	
	
	private String p0_Cmd;//业务类型
	private String p1_MerId;//商户编号
	private String p2_Order;//订单编号
	private String p3_Amt;//订单金额
	private String p4_Cur;//交易币种
	private String p5_Pid;//商品名称
	private String p6_Pcat;//商品种类
	private String p7_Pdesc;//商品描述
	private String p8_Url;//商户接收支付成功数据的地址
	private String p9_SAF;//送货地址
	private String pa_MP;//商户扩展信息
	private String pd_FrpId;//支付通道编码
	private String pm_Period;//订单的有效期
	private String pn_Unit;//订单有效期的单位
	private String pr_NeedResponse;//应答机制
	private String pt_UserName;//考生/学员姓名
	private String pt_PostalCode;//身份证号
	private String pt_Address;//地区 
	private String pt_TeleNo;//报考序号
	private String pt_Mobile;//手机号
	private String pt_Email;//邮件地址
	private String hmac;//签名数据
	
	//------ 以下是返回的参数---------
	
	private String r0_Cmd ;
	private String r1_Code;
	private String r2_TrxId;
	private String r3_Amt;
	private String r4_Cur;
	private String r5_Pid;
	private String r6_Order;
	private String r7_Uid;
	private String r8_MP;
	private String r9_BType;
	private String rb_BankId;
	private String ro_BankOrderId;
	private String rp_PayDate;
	private String rq_CardNo;
	private String ru_Trxtime;
	private String rq_SourceFee;
	
	//---退款--
	private String pb_TrxId;
	private String p5_Desc;
	
	/**
	 * @return the p0_Cmd
	 */
	public String getP0_Cmd() {
		return p0_Cmd;
	}
	/**
	 * @param p0_Cmd the p0_Cmd to set
	 */
	public void setP0_Cmd(String p0_Cmd) {
		this.p0_Cmd = p0_Cmd;
	}
	/**
	 * @return the p1_MerId
	 */
	public String getP1_MerId() {
		return p1_MerId;
	}
	/**
	 * @param p1_MerId the p1_MerId to set
	 */
	public void setP1_MerId(String p1_MerId) {
		this.p1_MerId = p1_MerId;
	}
	/**
	 * @return the p2_Order
	 */
	public String getP2_Order() {
		return p2_Order;
	}
	/**
	 * @param p2_Order the p2_Order to set
	 */
	public void setP2_Order(String p2_Order) {
		this.p2_Order = p2_Order;
	}
	/**
	 * @return the p3_Amt
	 */
	public String getP3_Amt() {
		return p3_Amt;
	}
	/**
	 * @param p3_Amt the p3_Amt to set
	 */
	public void setP3_Amt(String p3_Amt) {
		this.p3_Amt = p3_Amt;
	}
	/**
	 * @return the p4_Cur
	 */
	public String getP4_Cur() {
		return p4_Cur;
	}
	/**
	 * @param p4_Cur the p4_Cur to set
	 */
	public void setP4_Cur(String p4_Cur) {
		this.p4_Cur = p4_Cur;
	}
	/**
	 * @return the p5_Pid
	 */
	public String getP5_Pid() {
		return p5_Pid;
	}
	/**
	 * @param p5_Pid the p5_Pid to set
	 */
	public void setP5_Pid(String p5_Pid) {
		this.p5_Pid = p5_Pid;
	}
	/**
	 * @return the p6_Pcat
	 */
	public String getP6_Pcat() {
		return p6_Pcat;
	}
	/**
	 * @param p6_Pcat the p6_Pcat to set
	 */
	public void setP6_Pcat(String p6_Pcat) {
		this.p6_Pcat = p6_Pcat;
	}
	/**
	 * @return the p7_Pdesc
	 */
	public String getP7_Pdesc() {
		return p7_Pdesc;
	}
	/**
	 * @param p7_Pdesc the p7_Pdesc to set
	 */
	public void setP7_Pdesc(String p7_Pdesc) {
		this.p7_Pdesc = p7_Pdesc;
	}
	/**
	 * @return the p8_Url
	 */
	public String getP8_Url() {
		return p8_Url;
	}
	/**
	 * @param p8_Url the p8_Url to set
	 */
	public void setP8_Url(String p8_Url) {
		this.p8_Url = p8_Url;
	}
	/**
	 * @return the p9_SAF
	 */
	public String getP9_SAF() {
		return p9_SAF;
	}
	/**
	 * @param p9_SAF the p9_SAF to set
	 */
	public void setP9_SAF(String p9_SAF) {
		this.p9_SAF = p9_SAF;
	}
	/**
	 * @return the pa_MP
	 */
	public String getPa_MP() {
		return pa_MP;
	}
	/**
	 * @param pa_MP the pa_MP to set
	 */
	public void setPa_MP(String pa_MP) {
		this.pa_MP = pa_MP;
	}
	/**
	 * @return the pd_FrpId
	 */
	public String getPd_FrpId() {
		return pd_FrpId;
	}
	/**
	 * @param pd_FrpId the pd_FrpId to set
	 */
	public void setPd_FrpId(String pd_FrpId) {
		this.pd_FrpId = pd_FrpId;
	}
	/**
	 * @return the pm_Period
	 */
	public String getPm_Period() {
		return pm_Period;
	}
	/**
	 * @param pm_Period the pm_Period to set
	 */
	public void setPm_Period(String pm_Period) {
		this.pm_Period = pm_Period;
	}
	/**
	 * @return the pn_Unit
	 */
	public String getPn_Unit() {
		return pn_Unit;
	}
	/**
	 * @param pn_Unit the pn_Unit to set
	 */
	public void setPn_Unit(String pn_Unit) {
		this.pn_Unit = pn_Unit;
	}
	/**
	 * @return the pr_NeedResponse
	 */
	public String getPr_NeedResponse() {
		return pr_NeedResponse;
	}
	/**
	 * @param pr_NeedResponse the pr_NeedResponse to set
	 */
	public void setPr_NeedResponse(String pr_NeedResponse) {
		this.pr_NeedResponse = pr_NeedResponse;
	}
	/**
	 * @return the pt_UserName
	 */
	public String getPt_UserName() {
		return pt_UserName;
	}
	/**
	 * @param pt_UserName the pt_UserName to set
	 */
	public void setPt_UserName(String pt_UserName) {
		this.pt_UserName = pt_UserName;
	}
	/**
	 * @return the pt_PostalCode
	 */
	public String getPt_PostalCode() {
		return pt_PostalCode;
	}
	/**
	 * @param pt_PostalCode the pt_PostalCode to set
	 */
	public void setPt_PostalCode(String pt_PostalCode) {
		this.pt_PostalCode = pt_PostalCode;
	}
	/**
	 * @return the pt_Address
	 */
	public String getPt_Address() {
		return pt_Address;
	}
	/**
	 * @param pt_Address the pt_Address to set
	 */
	public void setPt_Address(String pt_Address) {
		this.pt_Address = pt_Address;
	}
	/**
	 * @return the pt_TeleNo
	 */
	public String getPt_TeleNo() {
		return pt_TeleNo;
	}
	/**
	 * @param pt_TeleNo the pt_TeleNo to set
	 */
	public void setPt_TeleNo(String pt_TeleNo) {
		this.pt_TeleNo = pt_TeleNo;
	}
	/**
	 * @return the pt_Mobile
	 */
	public String getPt_Mobile() {
		return pt_Mobile;
	}
	/**
	 * @param pt_Mobile the pt_Mobile to set
	 */
	public void setPt_Mobile(String pt_Mobile) {
		this.pt_Mobile = pt_Mobile;
	}
	/**
	 * @return the pt_Email
	 */
	public String getPt_Email() {
		return pt_Email;
	}
	/**
	 * @param pt_Email the pt_Email to set
	 */
	public void setPt_Email(String pt_Email) {
		this.pt_Email = pt_Email;
	}
	/**
	 * @return the hmac
	 */
	public String getHmac() {
		return hmac;
	}
	/**
	 * @param hmac the hmac to set
	 */
	public void setHmac(String hmac) {
		this.hmac = hmac;
	}
	/**
	 * @return the r0_Cmd
	 */
	public String getR0_Cmd() {
		return r0_Cmd;
	}
	/**
	 * @param r0_Cmd the r0_Cmd to set
	 */
	public void setR0_Cmd(String r0_Cmd) {
		this.r0_Cmd = r0_Cmd;
	}
	/**
	 * @return the r1_Code
	 */
	public String getR1_Code() {
		return r1_Code;
	}
	/**
	 * @param r1_Code the r1_Code to set
	 */
	public void setR1_Code(String r1_Code) {
		this.r1_Code = r1_Code;
	}
	/**
	 * @return the r2_TrxId
	 */
	public String getR2_TrxId() {
		return r2_TrxId;
	}
	/**
	 * @param r2_TrxId the r2_TrxId to set
	 */
	public void setR2_TrxId(String r2_TrxId) {
		this.r2_TrxId = r2_TrxId;
	}
	/**
	 * @return the r3_Amt
	 */
	public String getR3_Amt() {
		return r3_Amt;
	}
	/**
	 * @param r3_Amt the r3_Amt to set
	 */
	public void setR3_Amt(String r3_Amt) {
		this.r3_Amt = r3_Amt;
	}
	/**
	 * @return the r4_Cur
	 */
	public String getR4_Cur() {
		return r4_Cur;
	}
	/**
	 * @param r4_Cur the r4_Cur to set
	 */
	public void setR4_Cur(String r4_Cur) {
		this.r4_Cur = r4_Cur;
	}
	/**
	 * @return the r5_Pid
	 */
	public String getR5_Pid() {
		return r5_Pid;
	}
	/**
	 * @param r5_Pid the r5_Pid to set
	 */
	public void setR5_Pid(String r5_Pid) {
		this.r5_Pid = r5_Pid;
	}
	/**
	 * @return the r6_Order
	 */
	public String getR6_Order() {
		return r6_Order;
	}
	/**
	 * @param r6_Order the r6_Order to set
	 */
	public void setR6_Order(String r6_Order) {
		this.r6_Order = r6_Order;
	}
	/**
	 * @return the r7_Uid
	 */
	public String getR7_Uid() {
		return r7_Uid;
	}
	/**
	 * @param r7_Uid the r7_Uid to set
	 */
	public void setR7_Uid(String r7_Uid) {
		this.r7_Uid = r7_Uid;
	}
	/**
	 * @return the r8_MP
	 */
	public String getR8_MP() {
		return r8_MP;
	}
	/**
	 * @param r8_MP the r8_MP to set
	 */
	public void setR8_MP(String r8_MP) {
		this.r8_MP = r8_MP;
	}
	/**
	 * @return the r9_BType
	 */
	public String getR9_BType() {
		return r9_BType;
	}
	/**
	 * @param r9_BType the r9_BType to set
	 */
	public void setR9_BType(String r9_BType) {
		this.r9_BType = r9_BType;
	}
	/**
	 * @return the rb_BankId
	 */
	public String getRb_BankId() {
		return rb_BankId;
	}
	/**
	 * @param rb_BankId the rb_BankId to set
	 */
	public void setRb_BankId(String rb_BankId) {
		this.rb_BankId = rb_BankId;
	}
	/**
	 * @return the ro_BankOrderId
	 */
	public String getRo_BankOrderId() {
		return ro_BankOrderId;
	}
	/**
	 * @param ro_BankOrderId the ro_BankOrderId to set
	 */
	public void setRo_BankOrderId(String ro_BankOrderId) {
		this.ro_BankOrderId = ro_BankOrderId;
	}
	/**
	 * @return the rp_PayDate
	 */
	public String getRp_PayDate() {
		return rp_PayDate;
	}
	/**
	 * @param rp_PayDate the rp_PayDate to set
	 */
	public void setRp_PayDate(String rp_PayDate) {
		this.rp_PayDate = rp_PayDate;
	}
	/**
	 * @return the rq_CardNo
	 */
	public String getRq_CardNo() {
		return rq_CardNo;
	}
	/**
	 * @param rq_CardNo the rq_CardNo to set
	 */
	public void setRq_CardNo(String rq_CardNo) {
		this.rq_CardNo = rq_CardNo;
	}
	/**
	 * @return the ru_Trxtime
	 */
	public String getRu_Trxtime() {
		return ru_Trxtime;
	}
	/**
	 * @param ru_Trxtime the ru_Trxtime to set
	 */
	public void setRu_Trxtime(String ru_Trxtime) {
		this.ru_Trxtime = ru_Trxtime;
	}
	/**
	 * @return the rq_SourceFee
	 */
	public String getRq_SourceFee() {
		return rq_SourceFee;
	}
	/**
	 * @param rq_SourceFee the rq_SourceFee to set
	 */
	public void setRq_SourceFee(String rq_SourceFee) {
		this.rq_SourceFee = rq_SourceFee;
	}
	/**
	 * @return the pb_TrxId
	 */
	public String getPb_TrxId() {
		return pb_TrxId;
	}
	/**
	 * @param pb_TrxId the pb_TrxId to set
	 */
	public void setPb_TrxId(String pb_TrxId) {
		this.pb_TrxId = pb_TrxId;
	}
	/**
	 * @return the p5_Desc
	 */
	public String getP5_Desc() {
		return p5_Desc;
	}
	/**
	 * @param p5_Desc the p5_Desc to set
	 */
	public void setP5_Desc(String p5_Desc) {
		this.p5_Desc = p5_Desc;
	}
	
	
	
	
	

}
