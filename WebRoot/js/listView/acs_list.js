...
$(function(){
	...
	//判断时间
	$(".phone_main_01_txt:eq(2) .phone_main_01_txt_ret_01 a").each(function(){
		//判断如果当前遍历对象在默认查询里面有，那么就摸你点击查询加载数据
...
function screen(){
	...
	//判断手机分类是否为空，如果为空传入配件这个词
	if(!param.catIds || param.catIds.length<=0){
		param["catNames"] = "配件";
	}else{
		param["catNames"] = null;
	}
		...
		success: function(d){
			...
			if(d!=null){
				...
				$.each(d.goodsList,function(i,g){
					...
					var url = d.urlType+"/detail/"+g.specCode+".html";
					...
					$.each(g.goodsSkus,function(i2,sku){
						...
						if(i2==0){
							...
							if(sku.price){
								...
								//遍历取最小的价格
								if(price>0 && parseFloat(sku.price)<price){
									price = parseFloat(sku.price);
								}
							}
							if(sku.color){
